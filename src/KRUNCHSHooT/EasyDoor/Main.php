                    ...
                    if(!$player->isSneaking() || $item->isNull()){
                        if ($block->isHingeRight()) {
                            ...
                            if ($next instanceof Door) {
                                ...
                                if ($other instanceof Door && $other->hasSameTypeId($next) && !($other->isHingeRight()) && ($block->getFacing() === $other->getFacing())) {
                            ...
                            if ($next2 instanceof Door) {
                                ...
                                if ($other instanceof Door && $other->hasSameTypeId($next2) && ($other->isHingeRight()) && ($block->getFacing() === $other->getFacing())) {
                                    $other->setOpen(!$block->isOpen());
